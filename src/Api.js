...
const myGov = async () => {
    const res = await fetch('https://www.mygov.in/sites/default/files/covid/covid_state_counts_ver1.json');
    const data = await res.json();
    ...
    const response = new Array();
    Object.keys(data["Name of State / UT"]).forEach(key => {
        let temp = {
			"confirmed": data["Total Confirmed cases"][key],
			"deaths": data["Death"][key],
            "recovered": data["Cured/Discharged/Migrated"][key],
            "active": data["Active"][key],
			"deltaconfirmed": data["diff_confirmed_covid_cases"][key],
			"deltadeaths": data["diff_death"][key],
			"deltarecovered": data["diff_cured_discharged"][key],
            "deltaactive": data["diff_active_covid_cases"][key],
            "covid_portal_url": data["covid_portal_url"][key],
            "covid_facilities": data["covid_facilities"][key],
            "whatsapp_chatbot_url": data["whatsapp_chatbot_url"][key],
            "fb_chatbot_url": data["fb_chatbot_url"][key],
			"lastupdatedtime": data["updated_on"],
			"state": data["Name of State / UT"][key],
			"statecode": data["abbreviation_code"][key]
        };

        india_specific["confirmed"] += parseInt(temp["confirmed"]);
        india_specific["deaths"] += parseInt(temp["deaths"]);
        india_specific["recovered"] += parseInt(temp["recovered"]);
        india_specific["active"] += parseInt(temp["active"]);
        india_specific["deltaconfirmed"] += parseInt(temp["deltaconfirmed"]);
        india_specific["deltadeaths"] += parseInt(temp["deltadeaths"]);
        india_specific["deltarecovered"] += parseInt(temp["deltarecovered"]);
        india_specific["deltaactive"] += parseInt(temp["deltaactive"]);

        response.push(temp);
    });
    response.unshift(india_specific)
    return Promise.all(response);
}
